...
def products():
    ...
    # list(map(lambda x: x.removesuffix(".json"), os.listdir("app/data/opinions")))
    #  
    products = []
    for file in files: 
    ...
    return render_template('productlist.html.jinja', products=products)

@app.route('/product/<code>')
def product(code):
    opinions = pd.read_json(f"./app/data/opinions/{code}.json")
    return render_template('product.html.jinja', product_code=code, opinions=opinions.to_html(header=True, table_id="opinionsList", classes="table table-striped table-hover"))
